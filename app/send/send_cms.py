...
@router.callback_query(k.Menu_callback.filter(F.menu == 'send_cms'))
async def send_cms(call: CallbackQuery, callback_data: k.Menu_callback, state: FSMContext):
    await state.set_state(CMS.file)
...
@router.message(CMS.file, F.document)
async def handle_file(message: Message, state: FSMContext):
    ...
    file = FSInputFile(path=file_path, filename='CMS.txt')
